...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $studentsId = $_POST['selected_students_id'];

    // เตรียมคำสั่ง SQL สำหรับบันทึกหัวข้อและคำถาม
    $sqlEvaluation = "INSERT INTO evaluation_students (students_id, evaluation_name, score, evaluation_date, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW(), NOW())";
    $sqlActivity = "INSERT INTO evaluation_activity_student (id, activity_id, evaluation_name, evaluation_score) VALUES (?, ?, ?, ?)";
    ...
    try {
        ...
        foreach ($_POST['topics'] as $topic) {
            ...
            // บันทึกหัวข้อลงในตาราง evaluation_students
            $stmtEvaluation->bind_param("iss", $studentsId, $topicName, $scoreStr);
            ...
            if (isset($topic['questions']) && is_array($topic['questions'])) {
                foreach ($topic['questions'] as $question) {
                    ...
                    // สร้าง unique ID สำหรับ evaluation_activity_student
                    $uniqueId = uniqid('eval_', true);

                    // แปลง $activityId เป็น string
                    $activityIdStr = strval($activityId);

                    // บันทึกคำถามและคำตอบลงใน evaluation_activity_student
        ...
        // แสดงป๊อปอัพเมื่อบันทึกสำเร็จและเปลี่ยนเส้นทางกลับไปยัง evaluation_student.php
        echo "<script>
