...
<body class="bg-surface">
    <main>
            ...
            <div class="w-full page-wrapper xl:px-6 px-0">
                <div class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                    ...
                    ?>
                        ...
                        <form method="post" action="evaluation_student_insert.php" id="evaluationForm" class="p-6 space-y-6">
                        <div>
        <label for="studentSelect" class="block font-medium text-gray-700 mb-2">เลือกนักเรียน:</label>
        <select name="students_id" id="studentSelect" class="w-full p-2 mb-4 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
        ...
        <input type="hidden" name="selected_students_id" id="selectedStudentId">
    </div>
                            <script>
                                function showTeacherId() {
                                    const teacherSelect = document.getElementById('teacherSelect');
                                    const selectedTeacherId = teacherSelect.value; // รับค่า student_id ที่ถูกเลือก
                                    const displayDiv = document.getElementById('selectedTeacherId');

                                    if (selectedTeacherId) {
                                        displayDiv.textContent = "Teacher ID: " + selectedTeacherId; // แสดง student_id
                                    } else {
